// ibfs_hook6.c — ground-truth which of the 16 pooled buffers holds which camera.
//
...
#define DIR "/data/local/tmp/cap6"
#define LOG DIR"/map6.log"
#define NSLOT 16
...
int mq_read(void* thiz, void* out, unsigned long cnt){
  ...
  if(r && out && __sync_fetch_and_add(&g_fn,0)<g_maxfs){
    ...
    // One-shot: freeze all 16 slots at a single instant, with the descriptor that describes them.
    if(g_fn>=g_dumpat && !__sync_fetch_and_add(&g_dumped,0)){
      __sync_fetch_and_add(&g_dumped,1);
      for(int i=0;i<NSLOT;i++){
        if(!g_va[i] || !g_wh[i]) continue;
        char pth[96]; snprintf(pth,sizeof pth,DIR"/slot_%02d.gray",i);
        int f=open(pth,O_WRONLY|O_CREAT|O_TRUNC,0644);
        if(f>=0){ if(write(f,(const void*)(uintptr_t)g_va[i],g_wh[i])){} close(f); }
      }
      L("DUMP done at fs=%d: 16 slots frozen\n",g_fn);
    }
  }
  return r;
}
...
__attribute__((constructor)) static void init_(void){
  ...
  L("== ibfs_hook6 init ctor=%p mqread=%p pid=%d memfd=%d nslot=%d maxfs=%d ==\n",
