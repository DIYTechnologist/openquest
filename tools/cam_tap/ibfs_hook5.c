// ibfs_hook5.c — own the pixel mapping instead of borrowing Meta's.
//
...
#define DIR "/data/local/tmp/cap5"
#define LOG DIR"/map5.log"
#define NSLOT 16
...
__attribute__((constructor)) static void init_(void){
  ...
  L("== ibfs_hook5 init ctor=%p mqread=%p pid=%d memfd=%d nslot=%d maxfs=%d ==\n",
